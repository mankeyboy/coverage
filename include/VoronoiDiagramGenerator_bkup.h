...
public:
	...
	bool generateVoronoi(float *xValues, float *yValues, int numPoints, float minX, float maxX, float minY, float maxY, float minDist,vector<Point2d>& sites_);
	...
	bool getNext(float& x1, float& y1, float& x2, float& y2, int& siteid1, int& siteid2)
	{
		if(iteratorEdges == 0)
			return false;
		...
		y1 = iteratorEdges->y1;
		y2 = iteratorEdges->y2;
		siteid1 = iteratorEdges->siteid1;
		siteid2 = iteratorEdges->siteid2;
...
private:
	...
	struct Halfedge *ELleftbnd(Point2f *p);
	...
	int right_of(struct Halfedge *el,Point2f *p);

	struct Site *rightreg(struct Halfedge *he);
	struct Edge *bisect(struct	Site *s1,struct	Site *s2);
	float dist(struct Site *s,struct Site *t);
	struct Site *intersect(struct Halfedge *el1, struct Halfedge *el2, Point2f *p=0);
...
bool isOnLine(float x, float y, float x1, float x2, float y1, float y2, float sitex, float sitey);
