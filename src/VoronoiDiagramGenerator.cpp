...
bool VoronoiDiagramGenerator::generateVoronoi(vector<float> xValues, vector<float> yValues, int numPoints, float minX, float maxX, float minY, float maxY, float minDist)
{
	cleanup();
	cleanupEdges();
	int i,j;
	...
	//for(i=0;i<nsites;i++)
	//	cout<<"Site "<<i<<": "<<sites[i].coord.x<<" "<<sites[i].coord.y<<endl;
	return true;
}
...
struct Halfedge * VoronoiDiagramGenerator::ELleftbnd(Point2f *p)
{
	int i, bucket;
	struct Halfedge *he;
...
struct Site * VoronoiDiagramGenerator::intersect(struct Halfedge *el1, struct Halfedge *el2, Point2f *p)
{
	struct	Edge *e1,*e2, *e;
	struct  Halfedge *el;
	float d, xint, yint;
...
int VoronoiDiagramGenerator::right_of(struct Halfedge *el,Point2f *p)
{
	struct Edge *e;
	struct Site *topsite;
...
void VoronoiDiagramGenerator::endpoint(struct Edge *e,int lr,struct Site * s)
{
	...
	if(e -> ep[RE-lr]== (struct Site *) NULL) 
		return;
	//cout<<"Clip_line in endpoint"<<endl;
	clip_line(e);
...
Point2f VoronoiDiagramGenerator::PQ_min()
{
	Point2f answer;
...
/*
void VoronoiDiagramGenerator::GetClipPoint(int x1, int y1, int x2, int y2, float& resx, float& resy)
{
	//Assumes x1,y1 is outside
	
	if(abs(x1-x2)<5){
		int start_y = (y1<y2)?y1:y2;
		int end_y = y1+y2-start_y;
		int i;
		for(i=start_y;i<end_y;i++){
            if(cmp.grid[i][x1]==true){
                break;
            }
        }
        resy = i;
        ...
        int end_x = x1+x2-start_x;
        int j;
        for( j=start_x;j<=end_x;j++){
            if(cmp.grid[y1][j]==true){
                break;
            }
        }
        resx = j;
    	...
    	int line_x,line_y;
    	for(int r=1;r<(int)dist;r++){
    		line_x = (int)(x1 + r*cos(theta)); line_y = (int)(y1 + r*sin(theta));
            if(cmp.grid[line_y][line_x] == true){
                break;
            }
        }
        resx=line_x;
...
void VoronoiDiagramGenerator::pushGraphEdge(float x1, float y1, float x2, float y2)
{
	if(fabs(x1-x2)<0.5&&fabs(y1-y2)<0.5)
		//Trivial edges (between almost same points should be rejected)
		return;
	...
	/*if(cmp.grid[(int)y1][(int)x1]==false){

		float resx,resy;
		GetClipPoint(x1,y1,x2,y2,resx,resy);
		newEdge->x1 = resx;
		newEdge->y1 = resy;
	}
	else if(cmp.grid[(int)y2][(int)x2]==false){
		float resx,resy;
		GetClipPoint(x2,y2,x1,y1,resx,resy);
		newEdge->x2 = resx;
		newEdge->y2 = resy;
	}*/
...
{
	Point2f *s1 = (Point2f*)p1, *s2=(Point2f*)p2;
